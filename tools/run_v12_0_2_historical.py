"""Fixed, isolated launcher for the registered V12.0.2 historical attempt.
...
def main() -> int:
    if sys.argv[1:] != [_FLAG]:
        print(f"Usage: python3.12 tools/run_v12_0_2_historical.py {_FLAG}")
        return 2
    if sys.implementation.name != "cpython" or sys.version_info[:2] != (3, 12):
        print("V12.0.2 requires the registered CPython 3.12 runtime.", file=sys.stderr)
        return 2
    if any(name in os.environ for name in _ROUTING_OVERRIDES):
        print(
            "V12.0.2 accepts only the repository default SMTP route.", file=sys.stderr
        )
        return 2
    ...
    try:
        ...
        from lotto649.v12_0_2_registered_attempt import main as registered_main
        ...
        print(
            "V12.0.2 stopped before returning a verified result; do not retry.",
            file=sys.stderr,
        )
        return 1
