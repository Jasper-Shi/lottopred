...
def test_deployed_operational_history_loads_only_from_frozen_authority():
    ...
    assert history.seal.file_sha256 == DEPLOYED_SEAL_SHA256
    ...
    assert operational_history_provenance(history) == {
        "epoch": INCIDENT_ID,
        "seal_sha256": DEPLOYED_SEAL_SHA256,
        ...
        "suffix_sha256": DEPLOYED_SUFFIX_SHA256,
        "suffix_head_sha256": DEPLOYED_SUFFIX_HEAD_SHA256,
...
def test_operational_history_does_not_accept_caller_pin_overrides(
    ...
    def fake_loader(repository, **authority):
        captured["repository"] = repository
        captured.update(authority)
        return sentinel

    monkeypatch.setattr(operational_history, "load_verified_history", fake_loader)
...
def test_live_prediction_carries_verified_training_history(tmp_path, monkeypatch):
    ...
    provenance = {"epoch": INCIDENT_ID, "seal_sha256": DEPLOYED_SEAL_SHA256}
    captured = []
